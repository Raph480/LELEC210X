...
void Spectrogram_Compute(q15_t *samples, q15_t *melvec) {
    ...
    for (int i = 0; i < MELVEC_LENGTH; i++) {
        ...
        // Extract start and stop indices from hz2mel_col_indices
        int start_index = hz2mel_col_indices[2 * i];
        int stop_index = hz2mel_col_indices[2 * i + 1];
        int num_vals = stop_index - start_index;  // Number of elements to process

        int j;
        // Process four elements per iteration using unrolling
        for (j = start_index; j + 3 < stop_index; j += 4) {
            q31_t hz_vals1 = *((q31_t*)&hz2mel_values[j]);   // Load first two values
            q31_t buf_vals1 = *((q31_t*)&buf[j]);            // Load corresponding buf values
            sum = __SMLAD(hz_vals1, buf_vals1, sum);         // Accumulate two multiplications

            q31_t hz_vals2 = *((q31_t*)&hz2mel_values[j + 2]); // Load next two values
            q31_t buf_vals2 = *((q31_t*)&buf[j + 2]);
            sum = __SMLAD(hz_vals2, buf_vals2, sum);         // Accumulate two more multiplications
        }

        // Handle the remaining 1–3 elements
        for (; j < stop_index; j++) {
            sum += (q31_t) hz2mel_values[j] * (q31_t) buf[j];
        }
