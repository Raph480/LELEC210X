...
def Spectrogram_Compute(samples, melvec, window, hz2mel_mat, melvec_length = 20, samples_per_melvec = 512, dtype = np.int16):
    """
    Args:
    ...
    """
    num_bits = dtype(0).nbytes * 8

    # Buffers
    buf = np.zeros(samples_per_melvec, dtype=dtype)
    buf_fft = np.zeros(2 * samples_per_melvec, dtype=dtype)

    # STEP 1 : Windowing of input samples --> Pointwise product
    #vector_mult(samples, hamming_window, buf, SAMPLES_PER_MELVEC)
    buf = (np.int32(samples) * np.int32(window) >> (num_bits-1)).astype(dtype)  # Perform multiplication in int32
    ...
    for i in range(samples_per_melvec):
        buf[i] = ((np.int32(buf_fft[i]) << (num_bits-1)) // np.int32(vmax)).astype(dtype)
    ...
    for i in range(samples_per_melvec//2):
        ...
        buf[i] = np.sqrt(np.int16(np.int64(acc0 + acc1) >> (num_bits+1))<< (num_bits-1)).astype(dtype)

    # STEP 3.4 Denormalize the magnitude
    for i in range(samples_per_melvec//2):
        buf[i] = (np.int32(buf[i]) * vmax >> (num_bits-1)).astype(dtype)
    ...
    for i in range(melvec_length):
        ...
        melvec[i] = (sum >> (num_bits - 1)).astype(dtype)
...
def melspectrogram(audio, N_melvec=20, melvec_length=20, samples_per_melvec=512, N_Nft=512, window_type="hamming", fs2=11025, dtype=np.int16):
    if window_type == "hamming":
        hamming_window = np.hamming(N_Nft)
        window = float2fixed(hamming_window, maxval = 1, dtype=dtype)
    else:
        raise ValueError("Window type not supported")

    # Hz to Mel conversion
    mel = librosa.filters.mel(sr=11025, n_fft=N_Nft, n_mels=melvec_length)  # WARNING: in MCU tables, sr = 11025
    print("WARNING: Mel filter bank is computed with sr = 11025")
    mel = mel[:,:-1]
    mel = mel/np.max(mel)
    hz2mel_mat = float2fixed(mel, maxval = 1, dtype=dtype)

    melspec = np.zeros([N_melvec, melvec_length], dtype=dtype)

    for i in range(N_melvec):
        samples = data[i*samples_per_melvec:(i+1)*samples_per_melvec]
        
        Spectrogram_Compute(samples, melspec[i], window, hz2mel_mat, melvec_length, samples_per_melvec, dtype)
...
if __name__ == "__main__":
    ...
    #samples = (np.sin(2 * np.pi * f * t / fs)* MAX_FOR_DTYPE).astype(DTYPE)

    data, fs = sf.read("audio_files/merged_sine.wav", dtype=np.int16)

    melspec = melspectrogram(data, N_melvec=N_MELVEC, melvec_length=MELVEC_LENGTH, samples_per_melvec=SAMPLES_PER_MELVEC, N_Nft=N_NFT, window_type=WINDOW_TYPE, fs2=11025, dtype=DTYPE)    
