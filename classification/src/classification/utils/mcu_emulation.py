...
def Spectrogram_Compute(samples, melvec, window, hz2mel_mat, melvec_length = 20, samples_per_melvec = 512, dtype = np.int16):
    ...
    for i in range(samples_per_melvec//2):
        ...
        buf[i] = np.sqrt(np.int64(np.int16(np.int64(acc0 + acc1) >> (num_bits+1)))<< (num_bits-1)).astype(dtype)
...
def melspectrogram(audio, N_melvec=20, melvec_length=20, samples_per_melvec=512, N_Nft=512, window_type="hamming", sr=11025, dtype=np.int16):
