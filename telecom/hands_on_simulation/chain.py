...
class Chain:
    ...
    snr_range: np.ndarray = np.arange(-10, 25,5)

    # Lowpass filter parameters
    numtaps: int = 100
    cutoff: float = 142000 #BIT_RATE * osr_rx / 2.0001  # or 2*BIT_RATE,...
...
class BasicChain(Chain):
    ...
    def demodulate(self, y: np.array) -> np.array:
        ...
        # Time vector for one symbol period (normalized)
        t = np.arange(R) / (R * B)

        # Reference waveforms for symbol 0 and symbol 1
        s0 = np.exp(1j * 2 * np.pi * (-fd) * t)  # For symbol 0
        s1 = np.exp(1j * 2 * np.pi * fd * t)     # For symbol 1

        # Compute the correlations for all symbols in one go
        ...
        # Non-coherent detection: Compare magnitudes of r0 and r1 for all symbols at once
        bits_hat = (np.abs(r1) > np.abs(r0)).astype(int)  # Vectorized detection
